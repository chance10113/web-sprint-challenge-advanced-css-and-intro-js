...
*/
function getArtistByIndex(array, position) {
  return `the artist at index ${position} is ${array[position].name}`;
}
// console.log('getArtistByIndex(array, index): ', getArtistByIndex(artists, 0));
...
function get20s(time) {
  const newArray = [];
  let i = "";
  for (i = 0; i <= artists.length - 1; i++) {
    if (artists[i].years === time) {
...
console.log("get20s(time): ", get20s("1901 - 1959"));

// const newArray = [];
//   let item = "";
//   for (item of artists.years){
//       if (item.includes(time)){
//           newArray.push(item);
//       }
//   } return newArray

// console.log('get20s(time): ', get20s("1959"));
...
// const newArray = ["Salvador Dali", "Frida Kahlo"];
//let item = "";
// for(let i = 0; i < artists.length; i++){
//   if(artists.includes(time)){
//     return newArray;
//   }
// }
...
console.log("removeArtist(array, index): ", removeArtist(artists, 0));
...
function addArtist(array) {
  array.push({
    ...
    bio:
      "Born Chance Meric Indigo Montoya Swartz to impoverished parents in 1997 Tulsa, Ok, he is known for his humor, wit, and soon to be stylish web-design. Unfortunately, he needs some practice before he can achieve true greatness.",
  });
  return array;
}

console.log("addArtist(array): ", addArtist(artists));
